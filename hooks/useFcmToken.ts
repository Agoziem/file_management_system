...
import { useGetAllNotifications, useGetUserUnreadNotifications } from "@/data/notifications";
...
const useFcmToken = () => {
  ...
  const { setHasChecked } = useHasChecked();
  const { refetch } = useGetUserUnreadNotifications();
  ...
  useEffect(() => {
    const setupListener = async () => {
      ...
      const unsubscribe = onMessage(m, (payload) => {
        ...
        setHasChecked(false); // Reset the hasChecked state to false.
        refetch(); // Refetch unread notifications to update the count.
        refetchNotifications(); // Refetch all notifications to ensure the latest data is available.
